  ...
  private function getFormattedResults($results) {
    ...
    if (count($results->getRows()) > 0) {
      ...
      foreach ($results->getRows() as $row) {
        $table .= '<tr>';
		$rowTitle=htmlspecialchars($row[0], ENT_NOQUOTES);
		$rowUrl=htmlspecialchars($row[2], ENT_NOQUOTES).htmlspecialchars($row[1], ENT_NOQUOTES);
		
          //foreach ($row as $cell) {
			  
            $table .= '<td>';
                   $table.= '<a href="' . $rowUrl . '">' . $rowTitle . '</a>';
                  $table .= '</td>';
         // }
        $table .= '</tr>';
      }
